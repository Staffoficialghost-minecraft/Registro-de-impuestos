...
import { collection, doc, setDoc, getDoc, getDocs, query, where, updateDoc, addDoc, orderBy } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
...
window.nav = (id) => { document.querySelectorAll('.view').forEach(e => e.classList.add('hidden')); document.getElementById(id).classList.remove('hidden'); };

window.showPanel = (id) => {
    document.querySelectorAll('.panel').forEach(e => e.classList.add('hidden'));
    document.getElementById(id).classList.remove('hidden');
    if(id==='p-taxes') loadTaxes();
    if(id==='p-users') loadUsers();
    if(id==='p-ranks') loadRanks();
    if(id==='p-messages') loadMessages();
};
...
window.joinClan=async()=>{
    ...
    const clanDoc=snap.docs[0],clanId=clanDoc.id,clanName=clanDoc.data().name,email=mkEmail(nt,clanName);
    ...
    }catch(e){toast(e.code==='auth/email-already-in-use'?"El Nametag ya existe en este clan":"Error al registrar","error")}
};

window.login=async()=>{
    const nt=gId('l-nametag'),clan=gId('l-clan'),pass=gId('l-password');
    if(!nt||!clan||!pass) return toast("Faltan datos","error");
    try{await signInWithEmailAndPassword(auth,mkEmail(nt,clan),pass)}catch(e){toast("Credenciales inválidas","error")}
};
...
onAuthStateChanged(auth,async u=>{
    ...
    nav('v-dashboard');
    document.getElementById('dash-layout').classList.add('hidden');
    document.getElementById('unverified-msg').classList.add('hidden');
    if(!state.dbUser.verified){document.getElementById('unverified-msg').classList.remove('hidden');return;}
    const clanSnap=await getDoc(doc(db,"clans",state.dbUser.clanId));
    state.clan={id:clanSnap.id,...clanSnap.data()};
    const rSnap=await getDocs(query(collection(db,"ranks"),where("clanId","==",state.clan.id)));
    state.ranks=rSnap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>a.order-b.order);
    const wSnap=await getDocs(query(collection(db,"weeks"),where("
